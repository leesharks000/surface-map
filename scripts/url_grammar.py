...
def main():
    ...
    ap.add_argument('--skip', default='node_modules,.git,dist,.vercel,.astro'); ap.add_argument('--check', action='store_true'); ap.add_argument('--no-links', action='store_true'); ap.add_argument('--exclude', default=''); ap.add_argument('--include-pdf', action='store_true')
    a = ap.parse_args()
    ...
    if a.sitemap_out:
        ...
        if a.include_pdf:
            for p in sorted(PAGES.rglob('*.pdf')):
